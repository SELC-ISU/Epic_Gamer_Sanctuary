...
public class Board extends JPanel implements Runnable, Constants{
	...
	public Board(int width, int height) {
		...
		if(playerName.toUpperCase().equals("GRACEY")) {
			score += 1000;
			JOptionPane.showMessageDialog(null,  "You unlocked the secret 1,000 point bonus!!", "1,000 Points", JOptionPane.INFORMATION_MESSAGE);
		}
	...
	public void addItem(Item i) {
		...
		public void paintComponent(Graphics g) {
			...
			if(lives == MIN_LIVES) {
				...
				g.drawString("Game Over! Did you make it onto the high score table?", getWidth() / 5, getHeight() - 20);
		...
		private class BoardListener extends KeyAdapter{
			public void keyPressed(KeyEvent e) {
				if(e.VK_SPACE == KeyEvent.VK_SPACE) {
					if(lives > MIN_LIVES) {
				...
				if(e.VK_LEFT == KeyEvent.VK_LEFT) {
					paddle.setX(paddle.getX() - 50);
				}
				if(e.VK_RIGHT == KeyEvent.VK_RIGHT) {
					paddle.setX(paddle.getX() + 50);
				}
			}
